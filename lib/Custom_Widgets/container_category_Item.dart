  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: FutureBuilder<List<CategoryItemResponse>>(
       ...
       future: ApiManager.getAllCategoryItems(),       // load data from Api:
      
       builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {    // loading data:
           
            return const Center(child: CircularProgressIndicator(
            color:Constant.mainColor,
            ));

          } else if (snapshot.hasError) {                           // if data has error:
           ...
           else if (!snapshot.hasData || snapshot.data!.isEmpty) {     // if data is empty:
          ...
          List<CategoryItemResponse> categoryList = snapshot.data!;       // store list of category:
                        ...
                        child: Container(
                          ...
                          child: FittedBox(
                            ...
                            child: Text(
                              ...
                              style: const TextStyle(fontSize: 15,color:Constant.blackColorDark),
                            ),
                          ),
                        )
                      );
